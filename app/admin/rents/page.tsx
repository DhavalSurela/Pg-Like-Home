...
export default async function RentsPage() {
  ...
  const [rentsResult, tenantsResult, roomsResult, bedsResult] = await Promise.all([
    supabase.from("rents").select("*").order("month", { ascending: false }).order("created_at", { ascending: true }),
  ...
  const error =
    rentsResult.error ?? tenantsResult.error ?? roomsResult.error ?? bedsResult.error;
  ...
  return (
    <div className="space-y-8">
      <div>
        <p className="text-xs font-semibold tracking-[0.18em] text-stone-400 uppercase">Rents</p>
        <h1 className="mt-3 text-4xl font-light tracking-tight text-stone-900 sm:text-5xl">Rents</h1>
      ...
      <RentsManager rents={rents} tenants={tenants} currentMonth={currentMonth} />
    </div>
  );
}
